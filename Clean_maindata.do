...
preserve
keep if multiple!=0
keep patid BestDate spatype
drop if spatype==""
bysort patid BestDate: gen count=_n
reshape wide spatype, i(patid BestDate) j(count)
...
summ flag
noi di "drop " r(N) " swabs  -  multiple in same time period, same spa, drop furthest from ideal"
drop if flag ==1
bysort patid: gen first=(_n==1)
summ first
noi di  " - leaving " _N " records from " r(sum) " paticipants" 
...
noi summ multiple if multiple!=0
noi di r(N) " duplicated swabs" 
noi list patid Sent DateTaken Received  org_timepoint timepoint BestDate accuracy spatype if multiple!=0, sepby(patid)
...
reshape wide spatype, i(patid timepoint) j(count)
split spatype1, parse(/) gen(spatype3)
split spatype2, parse(/) gen(spatype4)
drop spatype1 spatype2
rename spatype3* spatype*
...
noi summ multiple if multiple!=0
noi di r(N) " duplicated swabs" 
noi list patid org_timepoint timepoint BestDate accuracy spatype if multiple!=0, sepby(patid)
