...
save "E:\users\amy.mason\Staph\stset_data(spa)_even.dta", replace

**************
*mixture model
*from sarah's email of 26/08/14, abandoning other plans for

use "E:\users\amy.mason\Staph\stset_data(spa)_even.dta", clear
stmix, dist(ww) difficult pmix(knownaq)
*log lik = -1706

*looks reasonable. Lets try adding other variables to pmix, lambdas and overall.
* variables of interest = 



*antiprev
*** doesn't look sensible - taking antibiotics increases chance of slow rate?

* anti6mons 
*** true = increased chance of fast loss. Umm... not what was expected.

*both have almost idenitical loglik = -1737

*won't converge univaribly
*ethnic, student, healthcare emplotment, no of household member, age, gender, degrade


**** okay, let's accept knownaq as starting point. what adds on sensibly?

*antiprev + knownaqu
* still not behaving as expected
...
*this doesn't seem to have worked getting different answers to capture file :(
*Sarah says abandon.
use "E:\users\amy.mason\Staph\stset_data(spa)_even.dta", replace

stmix, pmix(knownaquis) lambda1(knownaquis) lambda2(knownaquis base2) gamma1(knownaquis) gamma2(knownaquis) noinit dist(ww)
		...
		foreach caa in "" "knownaquis" "base2" "knownaquis base2"{
			foreach daa in "" "knownaquis" "base2" "knownaquis base2"{
				foreach eaa in "" "knownaquis" "base2" "knownaquis base2" {
					...
					capture stmix, pmix(`aaa') lambda1(`baa') lambda2(`caa') gamma1(`daa') gamma2(`eaa') dist(ww) iterate(100) noinit
					...
					capture stmix, pmix(`aaa') lambda1(`baa') lambda2(`caa') gamma1(`daa') gamma2(`eaa') dist(ww) noinit iterate(100) difficult
...
*save "E:\users\amy.mason\Staph\subset(spaloss)_mix_results2_even.dta", replace
...
stset time2 if(recruit_neg_no!=1), fail(lossevent) id(patid3) origin(t0start)
